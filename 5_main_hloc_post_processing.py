...
class UV360:
    ...
    def reconstruct_from_2D_to_3D(self,output_folder):
        ...
        # local point cloud (colored by z)
            ...
            file_name_html = os.path.splitext(image_data['file_name'])[0] + ".html"
            output_folder_registration=os.path.join(output_folder,"2D_to_DD")
            os.makedirs(output_folder_registration, exist_ok=True)
            file_name_html_path=os.path.join(output_folder_registration, file_name_html)
    ...
    def calc_depth_plot(self,output_folder):
        ...
        for i, (image_id, image_data) in enumerate(self.data.items()):
            ...
            R_SVD=self.data[image_id]["R_SVD"]
            T_SVD=self.data[image_id]["T_SVD"]
            # points2_new= (points -T_SVD) @ R_SVD
            # fig = plt.figure()
            # ax = fig.add_subplot(111, projection='3d')
            # ax.scatter(points2_new[:, 0], points2_new[:, 1], points2_new[:, 2], c='b', s=5)
            # ax.set_xlabel('X')
            # ax.set_ylabel('Y')
            # ax.set_zlabel('Z')
            ...
            # fig = plt.figure()
            # xx = np.arange(len(depth_map_region.flatten()))
            # yy = depth_map_region
            # fig = plt.figure()
            # plt.scatter(xx, depth_map_region.flatten() / 0.024 * -0.83, c='r', s=5)
            # ax.set_title('3D Point Cloud from TXT')
            
            # fig = plt.figure()
            # plt.imshow(depth_map_region, cmap='gray')  # or use 'plasma', 'viridis' etc.
            # plt.title("Depth Map Region")
